...
export default function ResidentSignup() {
  ...
  const handleCreateAccount = async (e) => {
    ...
    try {
      // Step 1: Register the user (no role field — Base44 doesn't allow client role assignment)
      ...
      // Step 2: Log in immediately (no OTP step)
      await base44.auth.loginViaEmailPassword(s2.email, s3.password);

      // Step 3: Small wait for session to establish
      ...
      // Step 5: Create the resident profile entity
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-sand-dark p-8">
            ...
            {!showOtp && step === 2 && (
              <div className="space-y-4">
                ...
                <FormField label="Street Address *" error={s2Errors.street_address}>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <FormField label="Unit / Apt # (optional)">
                    <input type="text" value={s2.unit_number} onChange={e => setS2({...s2, unit_number: e.target.value})}
                      placeholder="e.g. 4B" className={inputCls(false)} />
                  </FormField>
                  <FormField label="City *" error={s2Errors.city}>
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <FormField label="ZIP Code *" error={s2Errors.zip_code}>
                ...
                <FormField label="I am a *" error={s2Errors.resident_type}>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <FormField label="Primary Phone *" error={s2Errors.primary_phone}>
                  ...
                  <FormField label="Secondary Phone (optional)">
                    <input type="tel" value={s2.secondary_phone} onChange={e => setS2({...s2, secondary_phone: e.target.value})}
                      placeholder="(786) 555-0200" className={inputCls(false)} />
                  </FormField>
                </div>

                <FormField label="Email Address *" error={s2Errors.email}>
              ...
              <form onSubmit={handleCreateAccount} className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <FormField label="First Name *" error={fieldErrors.first_name}>
                    <input type="text" value={s3.first_name} onChange={e => setS3({...s3, first_name: e.target.value})}
                      placeholder="Jane" className={inputCls(fieldErrors.first_name)} />
                  </FormField>
                  <FormField label="Last Name *" error={fieldErrors.last_name}>
                    <input type="text" value={s3.last_name} onChange={e => setS3({...s3, last_name: e.target.value})}
                      placeholder="Smith" className={inputCls(fieldErrors.last_name)} />
                  </FormField>
                </div>

                <FormField label="Password * (min 8 characters)" error={fieldErrors.password}>
                  <div className="relative">
                    <input type={showPw ? "text" : "password"} value={s3.password}
                      onChange={e => setS3({...s3, password: e.target.value})} className={inputCls(fieldErrors.password)} />
                ...
                <FormField label="Confirm Password *" error={fieldErrors.confirm_password}>
                  <div className="relative">
                    <input type={showPw2 ? "text" : "password"} value={s3.confirm_password}
                      onChange={e => setS3({...s3, confirm_password: e.target.value})} className={inputCls(fieldErrors.confirm_password)} />
                ...
                <button type="submit" disabled={loading}
                  className="w-full bg-navy hover:bg-navy-mid text-white font-bold py-3 rounded-xl text-sm disabled:opacity-60 flex items-center justify-center gap-2">
            ...
            {!showOtp && step === "confirmed" && (
              <div className="text-center py-6 space-y-4">
                ...
                <a href="/portal/resident/dashboard"
                  className="inline-flex items-center gap-2 bg-navy text-white font-bold px-6 py-3 rounded-xl text-sm hover:bg-navy-mid transition-colors">
